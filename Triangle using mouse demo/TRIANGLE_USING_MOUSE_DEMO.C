...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{   
    #define NR_POINTS_IN_ARRAY      4 
    #define NR_POINTS_IN_TRIANGLE   3 
    ...
    {   
        ...
        case WM_LBUTTONDOWN: 
            ...
            if(iCnt == 2){
                ...
                PushBack(
                    pPointVector, 
                    pPointVector->pPoint[pPointVector->N-3].x, 
                    pPointVector->pPoint[pPointVector->N-3].y
                );  
            }

            iCnt = (iCnt + 1) % (NR_POINTS_IN_TRIANGLE);  
            if(bVar)
